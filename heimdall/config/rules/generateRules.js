...
const rulesInputFile = path.join(__dirname, "dev.rules.yaml");
const rulesOutputFile = path.join(__dirname, process.env.ENV+".rules.yaml");
...
if (
  ...
  console.error(
    "Please set the following environment variables: PUBLIC_ROOT_URL, HASURA_DOMAIN, FN_DOMAIN, NEXTJS_DOMAIN, KRATOS_DOMAIN"
  );
  return;
}

// Read the contents of the input file
fs.readFile(rulesInputFile, "utf8", (err, data) => {
  if (err) {
    console.error(err);
    return;
  }
  ...
  fs.writeFile(rulesOutputFile, replacedContent, "utf8", (err) => {
    ...
    console.log(
      `Search and replace operation completed. Output written to ${rulesOutputFile}.`
    );
  });
});
