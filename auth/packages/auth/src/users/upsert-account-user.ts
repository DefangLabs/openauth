...
export async function upsertAccountUser(
  account: Pick<Accounts, "id" | "name" | "email" | "extra">
) {
  // TODO: Check if user exists with account exists, else create user
  const { data: queryData, errors: queryErrors } = await hasuraAdminClient(
    QueryUsersForAccount,
  ...
  const { data: upsertData, errors: upsertErrors } = await hasuraAdminClient(
    UpsertAccountUserMutation,
    {
      object: {
        id: userId,
        name: account.name,
      ...
      onConflict: {
        ...
        updateColumns: [UsersUpdateColumn.UpdatedAt],
      },
    }
  );
  ...
  analytics.identify({
    ...
    traits: {
      email: user.email,
      name: user.name,
      accounts: user.usersUserAccounts.map(
        ({ userAccountsAccount }) => userAccountsAccount.provider
      ),
    },
  });
